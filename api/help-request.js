export default async function handler(req,res){
  ...
  let result;try{result=await fetch(`${url}/rest/v1/help_requests`,{method:'POST',headers:{apikey:service,'Content-Type':'application/json','Content-Profile':'beautiful_you',Prefer:'return=minimal'},body:JSON.stringify(payload)});}catch{return res.status(503).json({error:'The secure request service is temporarily unavailable. Please try again later when it is safe.'});}
  if(!result.ok) return res.status(500).json({error:'We could not send your request. Please try again when safe.'});
  return res.status(200).json({message:'Your request was submitted for review. This form is not monitored as an emergency service, and submission does not guarantee an immediate response. If you are in immediate danger, contact local emergency services when safe.'});
}
