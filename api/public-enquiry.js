...
export default async function handler(req,res){
  ...
  let result;try{result=await fetch(`${url}/rest/v1/${config.table}`,{method:'POST',headers:{apikey:service,'Content-Type':'application/json','Content-Profile':'beautiful_you',Prefer:'return=minimal'},body:JSON.stringify(config.map(Object.fromEntries(Object.entries(input).map(([key,value])=>[key,typeof value==='string'?value.trim():value]))))});}catch{return res.status(503).json({error:'The enquiry service is temporarily unavailable. Please try again shortly or contact us by email.'});}
